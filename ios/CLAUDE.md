...
オフライン対応の基盤としてGRDB（SQLite）のローカルDBを持つ。「Offline Sync (Beta)」設定（デフォルトOFF）がONのとき、メモのCRUDはローカルDB + Outbox経由になり、`Shared/Sync/` の SyncEngine（actor、push→pull）がサーバーと収束させる。OFFでは従来のオンライン専用挙動のまま。

- **同期層**: `SyncTransport`（通信protocol、テストはモック）/ `SyncEngine`（push→pull、カーソル管理）/ `SyncChangeApplier`（変更フィードのGRDB適用）/ `SyncScheduler`（起動・オンライン復帰・書き込み後・pull-to-refreshでトリガ、直列化）。競合ルールとサーバー仕様は `docs/architecture.md` の「同期アーキテクチャ」が正
