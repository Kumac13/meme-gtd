...
- **`appMode`（Storage Mode、デフォルト server）**: `standalone` にするとサーバー通信ゼロでメモ機能が端末内DBで完結する（`LocalMemoDataSource`、Outboxなし）。未対応ドメイン（タスク/記事/検索/プロジェクト）は `StandaloneDataSources.swift` の安全実装（空レスポンス + 英語エラー）。メモCRUD本体は `LocalMemoStore`（Outboxを知らない）として OfflineFirst 実装と共有
