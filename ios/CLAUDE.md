...
- **オフラインUI**: `Views/Components/OfflineReadOnlyBadge.swift` の `ConnectivityMonitor`（@MainActor、NWPathMonitor）+ Read-only ピル。読み取り専用の判定は `ConnectivityMonitor.isOfflineReadOnly`（**Serverモード かつ** offlineSyncEnabled かつ オフライン）に一元化 — Standalone は全機能ローカルのため読み取り専用にならない。タスク/記事画面でピル表示 + 編集UI無効化（メモ画面は編集可のため出さない）
