...
- `Issue/IssueDetailContentComponents`: 本文・コメントカード、操作メニュー、コメントとactivityの混在timeline
...
- `ImageAttachmentPresentation`: 画像picker、サイズ選択、upload、Markdown参照追記のpresentation coordinator
...
Task / Article / Templateの本文は`IssueContentCard`、Task / Articleのコメントとactivityは`IssueTimeline`、各項目のCopy / Edit / Deleteは`IssueItemMenu`を使う。Memoの日付バケットtimelineは固有構造として残し、項目menuだけを共有する。TaskのTodo操作、Articleのoriginによる編集可否などはcallbackと入力値で差し込む。

Memo一覧とMemo / Task / Article Detailの画像添付は`ImageAttachmentCoordinator`と`imageAttachmentPresentation`を使う。画面側はcomposerの`Binding`だけを渡し、picker、サイズ選択、upload状態、Markdown追記を個別実装しない。
