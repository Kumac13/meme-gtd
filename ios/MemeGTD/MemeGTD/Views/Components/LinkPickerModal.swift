...
struct LinkPickerModal<VM: IssueDetailProvider>: View {
    ...
    var body: some View {
        IssueLinkPicker(
            issueTypeLabel: viewModel.issueTypeLabel,
            selectedIssues: selectedIssues,
            selectedURLs: selectedURLs,
            search: viewModel.searchIssues,
            addIssue: { item, type in
                await viewModel.createIssueLink(targetIssueId: item.id, linkType: type)
            },
            removeIssue: { selection in
                guard case .persisted(let id) = selection.id else { return }
                await viewModel.deleteIssueLink(id)
            },
            addURL: viewModel.createUrlLink,
            removeURL: { selection in
                guard case .persisted(let id) = selection.id else { return }
                await viewModel.deleteUrlLink(id)
            },
            onDismiss: onDismiss
        )
    }
