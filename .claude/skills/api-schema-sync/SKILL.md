...
詳細は `docs/architecture.md` の「API契約の同期チェーン」を参照。
...
最新の完全な対応表は `docs/architecture.md` を参照（12ファイルが同期対象）。
...
- memo→task昇格の本文整形は `GET /api/memos/{id}/promote-preview` が正 — クライアントに整形ロジックを複製しない
