...
- Zodスキーマと `packages/shared` の型は意図的に別物（sharedはドメイン型、ZodはHTTP契約）— 統合しない
- memo→task昇格の本文整形は `GET /api/memos/{id}/promote-preview` が正 — クライアントに整形ロジックを複製しない（唯一の例外: iOS Standalone の `PromoteEngine.swift`。整形仕様を変えるときは TS/Swift 両実装とパリティテストを同時更新 — `docs/architecture.md` 設計意図参照）
