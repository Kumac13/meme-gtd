...
5. **issuesテーブルは4タイプ共用**: `type` カラムが `memo` / `task` / `article` / `template`。番号空間が共通なので `#id` 一つで一意に解決できる。
...
| `Template.swift` | `templateSchemas.ts` |
...
iOS の SwiftUI 実装は、リソース別画面を共通コンポーネントから合成する。画面全体を一つの汎用 View にせず、同じ意味と見た目を持つ安定した部品だけを `MemeGTD/Views/Components/` に置く。

- `Foundation/`: モーダルヘッダー、フィードバック、ローディング、作成ボタンなど、ドメインに依存しない画面プリミティブ
- `Issue/`: 相対時刻、ラベルチップ、Issue 種別バッジ、詳細カードなど、Memo / Task / Article / Template が共有する表示プリミティブ
- `Forms/`: 作成フォームの遷移行、空選択表示、segmented picker。Task / Article / Template のフォームはこれらと共通の title / body / metadata section を合成する。metadata section が Label / Project 候補の取得、picker 表示、draft の確定・キャンセルを所有し、各作成画面は確定済みの選択値と保存処理だけを所有する
- `Pickers/`: 選択状態、セクション見出し、Issue 候補表示、外部 URL 入力。`IssueLinkPicker` が recent/search の非同期状態、debounce、空表示、URL 入力、処理中表示を一括所有する。保存済み Detail と作成待ち Task の差は selection / mutation callback のアダプターだけに限定する
- Issue 一覧: Task / Article / Template の上端基準一覧は `StandardIssueList` が行タップ、区切り線、追加読込を所有する。4種すべての filter bar、pull-to-refresh、検索終了処理は `IssueListFilterBar` と一覧用 View modifier を共有し、Memo / Task の検索モード切替、modal作成画面の作成ボタン領域、検索結果のcopy actionも同じ意味単位で共有する
- Memo の一覧: Memo は下端基準の時系列、日付区切り、作成後のscrollを持つため `StandardIssueList` の対象外とする。ただしfilter、refresh、search lifecycleなど意味が同じ周辺部品には参加させ、一覧全体をリソース種別の分岐で汎用化しない
- Memo の作成: Memo は title / metadata を先に入力する modal ではなく、一覧の `FloatingComposer` から即時作成するクイックキャプチャを維持する。`FloatingComposer` は Memo 作成と各 Detail のコメント入力で共有し、展開、入力可否、送信、画像添付入口の UI を一箇所に置く。Memo 固有の保存、Project filter からの関連付け、作成後の timeline scroll は `MemoListViewModel` / `MemoListView` に残す
- リソース別 View: データ取得、画面遷移、シート表示、リソース固有アクションを所有し、共通部品へ値と callback を渡す

`CreationPresentationCoordinator` は作成画面の presentation lifecycle を一箇所で管理する。Template chooser を使う Task / Article は chooser の dismissal 後に UUID 付きの作成要求を `sheet(item:)` へ渡し、Template の直接作成も同じ要求型を使う。選択・表示ごとに新しいフォーム identity を発行し、以前の blank / Template の `@State` を再利用させない。chooser のキャンセル時は保留 payload を破棄し、次回表示へ漏らさない。

`TemplateChooserSheet` は Blank／Template 選択だけでなく、Template を `IssueCreationDefaults` へ変換する責務も持つ。Task 一覧、Article 一覧、Task Detail は変換済み初期値だけを受け取り、本文・Label・Project の適用規則を呼び出し側へ複製しない。

Sheet 内の操作から親の navigation hierarchy を変更する場合は `DeferredSheetActionCoordinator` を使い、payload を保留して Sheet の `onDismiss` 後に遷移する。dismiss と `NavigationPath` 更新を同時に行うと空の遷移先が表示されるため、Detail 内のIssueリンクは必ずこの順序を通す。
...
保存済みDetailのIssue Link / 外部URL操作は`IssueRelationService`を単一の正とし、ViewModel adapterは`IssueRelationManaging`のdefault implementationを使う。Memo / Task / Articleで共通する取得、追加後の再取得、削除後の配列更新、picker item変換、検索、error・haptic lifecycleを共有し、activity再取得などリソース固有の副作用だけをhookに残す。

DetailのLabel / Project操作は`IssueMetadataService`を単一の正とし、ViewModel adapterは`IssueMetadataManaging`のdefault implementationを使う。Memo / Task / Article / Templateで共通する候補の独立並列取得、選択差分の適用、新規Labelの候補整合、確定後の再取得順序を共有する。一部の候補取得やmutationが失敗しても他の処理を継続し、resource本体、store、activityの更新内容だけをhookに残す。

Issue一覧ViewModelの追加読込とexport feedbackは`IssueListStateProviding`のdefault implementationを使う。Task / Memo / Article / TemplateはSwiftUIが直接監視する`@Published`状態、query構築、API response型、store適用を所有し、非同期lifecycleだけを共通実装へ渡す。

Detailが共有UIへ公開する機能は`IssueMetadataProvider`、`IssueLinkProvider`、`IssueBookmarkProvider`、`IssueCopyProvider`へ分割する。`IssueInfoSheet`はMetadataだけを必須とし、LinksとBookmarkはoptional capabilityとして受け取る。TemplateはMetadataとCopyだけを実装し、未対応機能のno-opを持たない。
...
Task / Article / Templateの一覧行は`IssueCellLayout`を使い、title、検索highlight、snippet、metadata行の配置を共有する。Label / Projectの複数選択modalは`MultiSelectPickerShell`を使い、header、scroll、固定search barを共有し、Label作成とNo Project行は各pickerに残す。
...
Web の作成画面は `IssueForm` を共通フォーム境界とし、Task / Memo / Article / Template は保存処理とリソース固有フィールドだけをBindingとして持つ。作成・編集ページの外枠は `FormPageLayout`、右側オーバーレイは `SidePanel` を使い、ページやモーダルごとにbackdrop・header・close操作を複製しない。

Templateを使うTask新規、Article新規、Task DetailのNew Taskは`TemplateCreationFlow`を共通の入口とする。`TemplateChooser`が対象別一覧取得、絞り込み、Template詳細取得、Label名からIDへの変換、Blank初期値を所有し、各画面は変換済み初期値をフォームへ渡すだけにする。

Task / Memo / Article / Template一覧は`ListPageLayout`で検索、作成導線、filter、件数、empty、results、paginationの配置を共有する。各filterのtrigger、popover、Clearは`FilterDropdown`、選択toggleは`ToggleFilterButton`、documentの外側クリックlistenerは`useOutsideClick`が所有する。リソース固有のURL query、filter内容、ItemList、Memo mobile timelineは各一覧に残す。

Markdown画像のpaste / drop / upload / cursor挿入は`MarkdownTextarea`が所有する。Issue form、comment、editable content、Project formは本文Bindingだけを渡す。DetailのCopy Allは`useCopyItemContent`、overflow操作は`ActionMenu`と`InlineDeleteConfirmation`を使い、画面ごとにclipboard feedback timerやbackdropを複製しない。

Label / Project管理欄は`ManagementSection`、Task / Projectの編集可能な日付cardは`EditableSectionCard`、Taskの作成・詳細日時入力は`ScheduleDateTimeFields`を使う。Link追加は保存済みIssueと作成待ちTaskの両方で`LinkCreationEditor`を使い、永続化と重複検証だけをadapter callbackに残す。

Calendarを含むIssue詳細の右パネルは`ItemDetailPanel`を単一の正とし、リソース別の専用Detail panelを増やさない。

本節と「iOS プレゼンテーション層」の境界は、`packages/web/tests/unit/ComponentBoundaries.test.ts`（Web CI で自動実行、iOS ソースの走査を含む）と `ios/MemeGTD/MemeGTDTests/ComponentBoundaryTests.swift`（Xcode テスト）のディレクトリ走査型構造テストが検査する。共通境界を追加・変更したら、両テストと `packages/web/CLAUDE.md`・`ios/CLAUDE.md` の選択表も更新する。
...
| Articles | `/api/articles`, `/api/articles/{id}`、`/{id}/bookmark`、`/{id}/unbookmark`、`/{articleId}/comments`、`/{articleId}/comments/{commentId}` |
