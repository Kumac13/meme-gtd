...
- **Storage Mode（Server / Standalone）**: iOS は App Group 設定 `appMode` で動作モードが決まる。初回解決は「インストール直後 = standalone。ただしアプリを消さずアップデートした既存端末（apiUrl が残存）= server」で、解決結果を永続化する。切替は Settings のピッカーで行う（Server モードはサーバーを直接読むため、切替に移行作業は不要）。端末内データのサーバーへの移動のみ Migrate to Server（バルクアップロード、冪等・一方向）。Standalone はサーバー通信ゼロで、メモ・タスク・キーワード検索・ラベル・リンクが端末内 DB で完結する。ローカル CRUD 本体は `LocalMemoStore` / `LocalTaskStore` / `LocalCommentStore`（Outbox を知らない純粋なローカル CRUD 層）として OfflineFirst 実装と共有する。未対応ドメイン（記事/プロジェクト/セマンティック検索）は空レスポンス + 英語エラーの安全実装。Standalone の削除はハード削除（サーバー到達経路がなく tombstone の回収先がないため — Standalone→Server 移行の設計時に要考慮）
