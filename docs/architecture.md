...
保存済みDetailのIssue Link / 外部URL操作は`IssueRelationService`を単一の正とする。Memo / Task / Articleで共通する取得、追加後の再取得、削除後の配列更新、picker item変換をserviceが所有し、activity再取得、error、hapticなどリソース固有の副作用は各ViewModelに残す。

DetailのLabel / Project操作は`IssueMetadataService`を単一の正とする。Memo / Task / Article / Templateで共通する候補の独立並列取得、選択差分の適用、新規Labelの候補整合をserviceが所有する。一部の候補取得やmutationが失敗しても他の処理を継続し、resource本体、store、activityの再取得は各ViewModelに残す。
...
Task / Memo / Article一覧のProject絞り込みは`ProjectFilterDropdown`を使う。開閉、外側クリック、Clear、No Project、選択表示はコンポーネントが所有し、各一覧はURL filter更新callbackだけを渡す。Calendarを含むIssue詳細の右パネルは`ItemDetailPanel`を単一の正とし、リソース別の専用Detail panelを増やさない。
