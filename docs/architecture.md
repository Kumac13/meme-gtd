...
- **タスク/記事/プロジェクトは閲覧専用キャッシュ**（メモだけがオフラインで読み書き可）。読みはリモート優先で、サーバー到達不能（ネットワークエラー）のときのみローカルフォールバック。書き込みは到達不能時にエラーとし、UI は編集操作を無効化してオフラインバナーを表示する。task/article の行は差分プルが `issues` に保存するためフォールバック元は同期フィードそのもの（REST レスポンスの書き戻しはしない — 同期の帳簿と矛盾させないため）
