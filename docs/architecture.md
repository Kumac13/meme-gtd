...
- `Forms/`: 作成フォームの遷移行、空選択表示、segmented picker。Task / Article / Template のフォームはこれらと共通の title / body / metadata section を合成する。metadata section が Label / Project 候補の取得と picker 表示を所有し、各作成画面は選択値と保存処理だけを所有する
- `Pickers/`: 選択状態、セクション見出し、Issue 候補表示、外部 URL 入力。保存済みリンクと作成待ちリンクの mutation は呼び出し側に残す
- リソース別 View: データ取得、画面遷移、シート表示、リソース固有アクションを所有し、共通部品へ値と callback を渡す

Template chooser から作成フォームへ遷移するときは、chooser の dismissal 後に UUID 付きの作成要求を `sheet(item:)` へ渡す。選択ごとに新しいフォーム identity を発行し、以前の blank / Template の `@State` を再利用させない。
