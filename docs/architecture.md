...
- **Storage Mode（Server / Standalone）**: iOS は App Group 設定 `appMode`（デフォルト server）で動作モードを切り替える。Standalone はサーバー通信ゼロで、メモの CRUD 本体を `LocalMemoStore`（Outbox を知らない純粋なローカル CRUD 層）として `OfflineFirstMemoDataSource` と共有する。Standalone の未対応ドメイン（タスク/記事/検索/プロジェクト）は空レスポンス + 英語エラーの安全実装。Standalone の削除はハード削除（サーバー到達経路がなく tombstone の回収先がないため — Standalone→Server 移行の設計時に要考慮）
