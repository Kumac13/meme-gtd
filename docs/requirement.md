...
- **iOS の Storage Mode**（オフライン対応）:
  ...
  - **Standalone モード**: サーバー不要。メモ機能（CRUD/ブックマーク/コメント/一覧フィルタ）が端末内 DB で完結する。タスク/記事/検索/プロジェクトは段階的に対応予定（未対応機能は英語の案内を表示）。Standalone→Server の一方向移行のみ将来サポートする（逆方向・複数端末マージは非対応）。
