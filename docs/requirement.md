...
- **iOS の Storage Mode**（オフライン対応）:
  ...
  - **モード切替**: Settings のピッカーで切り替えられる。Server にするには URL を設定して切り替えるだけ（データはサーバーを直接読むため、入れ直した端末でも移行作業は不要）。Standalone で作った端末内データをサーバーへ移す場合のみ「Migrate to Server」（バルクアップロード、冪等・一方向）を使う。
