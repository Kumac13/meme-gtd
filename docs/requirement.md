...
- **iOS の Storage Mode**（オフライン対応）:
  - **初期モード**: 新規インストール（apiUrl 未設定）は Standalone で開始し、サーバーなしで即使える。既存インストール（apiUrl 設定済み = サーバー利用者のアップグレード）は Server を維持し挙動不変。
  - **モード遷移は一方向のみ**: Standalone→Server は設定の「Migrate to Server」（全データのバルクアップロード）でのみ行う。Server→Standalone への切替は提供しない。
