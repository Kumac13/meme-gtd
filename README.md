...
1. **Ollamaのインストール**: `brew install ollama`
2. **Ollamaサーバーの起動**: `ollama serve`
3. **embeddingモデルのダウンロード**: `ollama pull qwen3-embedding:4b`
...
**注意:** embeddingサーバーが起動していない場合、セマンティック検索は503を返します。他の機能は全て正常に動作します。
...
# セマンティック検索（ベクトル類似度、embeddingサーバー必須）
...
| `mgtd embedding sync` | 全issueのベクトル埋め込み生成/更新（`--model`, `--json`） |
