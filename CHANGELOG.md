...
- iOS のオフライン判定を端末のネットワーク状態ではなくサーバー到達性で行うよう修正。
  ...
  - 判定は「サーバーに届くか」のみで行い、端末のネットワーク状態は一切参照しない。実際の API リクエストの結果を主シグナルとし（HTTP レスポンス受信 = 到達可、トランスポート失敗 = 到達不能。リクエストキャンセルは判定に使わない）、オフライン判定中のみ 30 秒間隔で `GET /api/health` により復帰を検知する。
  - あわせて、`APIClient.delete` がトランスポート失敗時に生の `URLError` を投げるため、オフライン時の削除操作が Read-only エラーに分類されない問題を修正。
