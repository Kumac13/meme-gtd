...
- iOS のオフライン判定を端末のネットワーク状態ではなくサーバー到達性で行うよう修正。
  - 従来は NWPathMonitor（端末のネットワーク経路の有無）だけで判定していたため、端末はオンラインでもサーバーに届かない状況（Tailscale トンネル断・サーバー停止など）で Read-only 表示が出ず、編集操作が失敗していた。
  - 判定は「サーバーに届くか」のみで行う。実際の API リクエストの結果を主シグナルとし（HTTP レスポンス受信 = 到達可、トランスポート失敗 = 到達不能。リクエストキャンセルは判定に使わない）、単発の失敗では反転せず `GET /api/health` プローブの失敗で確認してからオフラインに切り替える（遅いサーバーへのタイムアウト 1 回で全体が Read-only にならないように）。
  - オフライン中は 30 秒間隔の自走ループで復帰を検知し、端末のネットワーク経路の変化（機内モード・Wi-Fi 切断/復帰・起動直後）では即時にプローブを撃つ。経路状態はプローブのトリガーとしてのみ使い、判定には使わない。
  - あわせて、`APIClient.delete` がトランスポート失敗時に生の `URLError` を投げるため、オフライン時の削除操作が Read-only エラーに分類されない問題を修正（キャンセルはネットワーク障害として分類しない）。JSON コピー機能で応答の整形失敗が「Network error」と表示される誤分類も修正。

## 0.52.0 - 2026-08-07

### New Features

- Web / iOS の Task・Memo・Article 詳細に「Copy All Contents with JSON」を追加。
...
- Web / iOS のJSONコピーがTask・Memo・Article本体とコメントの日時データを保持することを検証するユニットテストを追加。
...
- iOS の Schedule フィルターで日付のドラムロールを開くとシートを自動的に画面上端まで展開し、操作中は大きい表示を維持するよう修正。通常時は Label / Project と同様に中・大の高さを手動で切り替えられるようにした。
...
- iOS Article Detail が初回ロード時のタイムライン更新で記事末尾へ自動スクロールし、長い記事で真っ白な領域を表示する問題を修正。初回表示は先頭を維持し、コメント送信時とComposer展開時だけ末尾へ移動するようにした。
...
- **Webの入力欄で長文入力時にカーソルが画面下へ押し出される問題を修正**: 自動拡張（`useAutoGrow`）が上限なしで textarea を伸ばし続け、かつ `overflow-hidden` で内部スクロールも無効だったため、Task/Memo・body/comment を問わず、内容が長くなるほどキャレットがビューポート下端へ押し出されて入力位置が見づらくなっていた。GitHubのコメント欄や autosize 系ライブラリと同じ一般的な方式に変更:
  - 自動拡張はビューポート比の max-height まで（フォーム・コメント欄は 60vh、モバイルのフローティング入力欄は 40vh、メモスレッドのインライン編集は 50vh）。上限到達後は textarea の内部スクロール（`overflow-y: auto`）に切り替え、以降のキャレット追従はブラウザ標準挙動に任せる。
  - 上限到達前の伸長中も、JSによる高さ変更ではブラウザがページをスクロールしないため、フォーカス中に高さが変わったら `scrollIntoView({ block: 'nearest' })` で入力欄下端を可視域に保つようにした。
  - あわせて、border 付き textarea（border-box）で高さが上下 border 分だけ不足していた計算を補正。

### Tests

- `useAutoGrow` のユニットテストを追加: scrollHeight への高さ追従、max-height クランプ時の内部スクロール有効化（非クランプ時は無効のまま）、フォーカス中の高さ変化時の追従スクロール（非フォーカス時・高さ不変時はスクロールしないこと）、値変更時の再計算を検証。

## 0.47.2 - 2026-07-08

### Bug Fixes

- **Webのリスト表示でマーカーと本文がずれる問題を修正**: Markdownのリストを `list-style-position: inside`（`list-inside`）+ `li` へのマージンで描画していたため、長文の折り返し行がマーカー（・や番号）の真下に回り込み、箇条書きの中に番号リストをネストした場合などにインデントが崩れて見えていた（Taskコメント等で顕在化。iOS版は元から問題なし）。GitHubと同様の `list-style-position: outside` + リスト側のパディングに変更し、折り返し行が本文の開始位置に揃うようにした。
- **Memo一覧で `>` 引用がイタリック表示になり、引用以降の本文が表示されない問題を修正**: 一覧プレビュー（`InlineMarkdownRenderer`）が blockquote をイタリックの span として描画しており、さらにプレビュー抽出が本文の1行目（最大150文字）しか対象にしていなかったため、引用で始まるメモは引用の続きや後続段落が一覧に一切表示されなかった。引用は Detail と同様の左ボーダー付き（非イタリック）スタイルで描画し、プレビューは改行を保持したまま先頭300文字を表示するようにした（超過分は `...` で切り詰め）。

### Tests

- Webユニットテストを追加: リストのマーカー位置（`list-outside` + パディング、`li` に余分なマージンがないこと）、ネストリストの構造、一覧プレビューの引用スタイル（非イタリック・左ボーダー）と引用後の段落表示、`extractPreview` の改行保持・切り詰め挙動を検証。
...
- **Web の「Copy Results」「Copy with Comments」がフィルタ全件をコピーできるように**: これまで Web はページネーション（タスク一覧の離散ページ、メモ一覧の無限スクロール）のせいで、コピーに送られるのは画面にロード済みのアイテムだけで、フィルタ期間の全メモを一括コピーできなかった（iOS はフィルタ全件を事前ロードするため一発でコピーできていた）。
  - `POST /api/search/export` に `scope`（`loaded` | `all`、既定 `loaded`）を追加。`all` のときサーバーが `filters` から同じ一覧／keyword 検索クエリをページ制限なしで再実行して全マッチを確定し、コメント全文込みで返す。フィルタ→マッチの対応をサーバー単一の正とし、クライアント側でフィルタ意味論を再実装しない設計。
  - Web は semantic 検索以外で `scope: 'all'` を送る。semantic 検索は類似度ランキングの top-K で「全件」が定義できないため、従来どおり表示中のアイテムを対象にする。
  - 暴発防止に上限（5000件）を設け、超過時は切り詰めて `truncated: true` を返し、Web は「Copied first N」警告を表示する（サイレントカットにしない）。
  - iOS はフィルタ全件を事前ロードするため挙動は変わらない（`scope` 未指定 = `loaded`）。

### Tests

- `scope="all"` の統合テストを追加: ラベル・日付・ステータスフィルタで itemIds を無視して全マッチを返すこと、コメント同梱、keyword 検索時のサーバー側スニペット再構築、semantic 検索の loaded フォールバック、`scope` 省略時の loaded 既定、アクティビティログの全件カウント記録を検証。

## 0.46.2 - 2026-07-06

### Bug Fixes

- **Web の三点リーダーメニューが操作後に開いたままになる問題を修正**: タスク・メモ詳細の本文カード（EditableContent）で Copy を実行してもメニューが閉じず、出っぱなしになっていた。Edit/Delete と同様に実行後メニューを閉じるようにした。
  - あわせて、外側クリックで閉じる仕組みがなかった三点リーダーメニュー（EditableContent / LinkItem / UrlLinkItem）に、ItemList・MemoDetail と同じバックドロップ方式のクリックアウトサイド処理を追加し、フロントエンド全体で挙動を統一した。
