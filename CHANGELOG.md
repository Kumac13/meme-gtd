...
- **iOS: オフライン表示がタスク/記事の詳細画面でタイトルに重なる問題を修正**: 詳細画面は独自ヘッダ（`ignoresSafeArea` + 手動インセット）のため `safeAreaInset` で挿入したピルがタイトルに重なっていた。詳細画面からはオフライン表示を撤去し（編集UIの無効化が状態を伝える）、一覧画面のみに残した。文言も "Offline — tasks are read-only" から "Offline" のみに短縮。

## 0.39.1 - 2026-07-03

### Bug Fixes

- **iOS: オフライン中のメモがオンライン復帰後にサーバーへ反映されないことがある問題を修正**: 復帰直後の同期は「ネットワーク復帰」イベント1回きりで起動していたが、Tailscale 利用時は Wi-Fi/セルラー復帰の時点で VPN トンネルがまだ確立しておらず push が失敗する。従来はその後の外部トリガ（画面遷移・書き込み等）が来るまで再送されず、同じ画面に留まると未送信のまま放置されていた（サーバーログで push 未到達を確認）。修正: 同期実行結果に Outbox 残数を持たせ、失敗または残がある場合は SyncScheduler が指数バックオフ（5s→15s→45s→120s上限）で自動再試行する。加えて再試行待機中にネットワーク経路が変化した場合（VPN トンネル確立など）は待たずに即時再実行する。
- **iOS: オフラインバナーが Liquid Glass の透過デザインを損なう問題を修正**: 全幅のマテリアル帯からフローティングのカプセル型ピル（競合トーストと同じ様式）に変更し、スクロールエッジの透過が保たれるようにした。

### Tests

- 失敗した同期実行が Outbox 残数を報告すること、外部トリガなしでバックオフ再試行が Outbox を消化することを検証する2テストを追加（iOS 計39件）。
...
- **iOS タスク・記事・プロジェクトのオフライン閲覧（オフライン同期 Phase 7）**: Offline Sync ON時、タスク・記事・プロジェクトの最終取得データをオフラインで閲覧できるようになった（編集は不可）。これでケース2（サーバー連携モードのオフライン対応）のスコープが完成。
  - 読み取りはサーバー優先で、サーバー到達不能（ネットワークエラー）のときだけローカルGRDBミラーにフォールバックする。タスク/記事の行は差分プルが `issues` に保存済みのため、追加の取得処理なしでキャッシュが最新に保たれる（4xx/5xxやデコード失敗はフォールバックせずそのまま表面化）。
  - ローカル一覧はサーバーと同じフィルタ意味論を再現: タスクは status / label（OR）/ bookmarked / タイトル検索 / スケジュール日付範囲（scheduled_start→actual_start フォールバック）、記事はタイトル・本文検索。プロジェクトフィルタ付きタスク一覧は部分キャッシュで誤った絞り込み結果を見せないため空ページを返す。
  - プロジェクトは同期フィード対象外（サーバー側ハード削除）のため、GRDBマイグレーション `002_projects_cache`（`projects` / `project_items`）を追加し、`GET /api/projects` 成功ごとに全量スナップショット、issue の所属プロジェクトは issue 単位で入れ替えキャッシュする。
  - オフライン時の編集操作は `OfflineReadOnlyError`（"Offline — this action requires a connection."）を返し、UI側もタスク作成ボタン・ステータス変更・コメント投稿・Info シートの各書き込み操作を無効化。タスク/記事画面の上部にオフラインバナー（"Offline — tasks are read-only" 等、NWPathMonitor ベース）を表示し、オンライン復帰で自動的に消えて編集が復活する。メモは従来どおりオフラインでも編集可。
  - Offline Sync OFF（デフォルト）時の挙動は完全に従来どおり。

### Tests

- `OfflineFirstReadCacheTests` を追加（8件、iOS 計37件）: タスクのフォールバック読みとフィルタ再現、プロジェクトフィルタの空ページ、getTask + コメントのローカル読み、記事のフォールバック（meta JSON 復元含む）、書き込みの readonly エラー（タスク/記事/プロジェクト）、projects スナップショットの全量入れ替えと issue 別メンバーシップキャッシュ。
