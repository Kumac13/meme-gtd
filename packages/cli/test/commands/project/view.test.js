...
describe('project view command', () => {
  let tmp, configPath, dbPath, env;

  test('setup test environment', () => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mgtd-project-view-'));
  ...
  test('view empty project', () => {
    const createProject = runCli(['project', 'create', 'Empty Project', '-j'], { env });
    const projectId = JSON.parse(createProject.stdout).id;

    const result = runCli(['project', 'view', String(projectId), '-j'], { env });
    assert.equal(result.status, 0, result.stderr);

    const output = JSON.parse(result.stdout);
    assert.equal(output.id, projectId);
    assert.ok(Array.isArray(output.items));
    assert.equal(output.items.length, 0);
  });

  test('view project with items', () => {
    const createProject = runCli(['project', 'create', 'Test Project', '-j'], { env });
    const projectId = JSON.parse(createProject.stdout).id;

    const createTask = runCli(['task', 'create', '-t', 'Task 1', '--no-editor', '-j'], { env });
    ...
    assert.equal(output.items[0].issue.id, taskId);
  });

  test('view non-existent project shows error', () => {
    const result = runCli(['project', 'view', '99999'], { env });
