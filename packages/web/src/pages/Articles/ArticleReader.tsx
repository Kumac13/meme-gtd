...
export const ArticleReader: React.FC = () => {
  ...
  const handleCopyAllWithJSON = async () => {
    await copyWithJSON({ item: article, comments });
  };
  ...
  const articleSidebarActions = (
    <>
      ...
      >
        ...
        <span className="text-sm">{copiedFormat === 'text' ? 'Copied!' : 'Copy All Contents'}</span>
      </button>

      <button
        onClick={handleCopyAllWithJSON}
        className="flex items-center gap-3 px-2 py-2 w-full text-left text-gray-700 hover:text-github-green-600 hover:bg-gray-50 rounded-md transition-colors"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span className="text-sm">{copiedFormat === 'json' ? 'Copied!' : 'Copy All Contents with JSON'}</span>
      </button>
    </>
  );
